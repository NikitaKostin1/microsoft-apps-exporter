...
func main() {
	...
	// Ensutes subscribed to Microsoft Graph API notificaitions.
...
func makeSignalNotify(cancel context.CancelFunc) <-chan struct{} {
	...
	go func() {
		<-signalChannel
		slog.Info("Received termination signal. Shutting down gracefully... 'context canceled' errorrs are expected. ")
		cancel()
		close(stop)
	}()
