...
func newSharepointHandler(syncer *sync.Syncer) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if r.Method != http.MethodPost {
			handleMethodNotAllowed(w, r.Method)
			return
		}
		...
		sharepoint := exctractConfig(siteID, listID)

		// Validate if the struct was found
		if sharepoint.ListID == "" || sharepoint.SiteID == "" {
		...
		go func() {
			if err := syncer.SyncSharepoint(sharepoint); err != nil {
...
func exctractConfig(siteID, listID string) models.ListConfig {
	config := configuration.GetConfig()

	for _, s := range config.Sharepoint.Lists {
		if s.SiteID == siteID && s.ListID == listID {
			return s
		}
	}
	return models.ListConfig{}
}
